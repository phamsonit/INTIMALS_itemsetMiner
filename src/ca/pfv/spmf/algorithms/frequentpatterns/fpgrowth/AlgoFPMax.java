...
public class AlgoFPMax {
	...
	//a special character used to separate @Item in the input file
	public static final String uniChar = "\u00a5";
	//minimum size of itemset
    private int minsize;
	 ...
	 */
	public Itemsets runAlgorithm(String input, String output, double minsupp, int _minsize) throws FileNotFoundException, IOException {
		...
		while( ((line = reader.readLine())!= null)){ 
			...
			if(line.charAt(0) == '@'){
			    //store item - string to itemString
                String[] temp = line.split(uniChar);
                if(temp.length > 1){
                    itemString.put(temp[1],temp[2]);
                }
                continue;
            }
