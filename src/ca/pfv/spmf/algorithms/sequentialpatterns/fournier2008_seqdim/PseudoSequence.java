	...
	// helper for the above method
	 List<List<Position>> getAllInstancesOfPrefixHelper(Sequence prefix, int indexItemset, 
		...
		for(int i=decalageItemset; i< size(); i++){
			...
			if(!firstTime && getTimeStamp(i)-itemsetShift != prefix.get(indexItemset).getTimestamp()){  // V�RIFIER D�CALAGE TEMPS
				continue;
			}
			int indexItem =0;
			...
			for(int j=0; j < getSizeOfItemsetAt(i); j++){
				...
				if(id == iDCourant){// l'item match
					...
					if(listPositions.size()+ listPositionsTotal.size()	== prefix.getItemOccurencesTotalCount())  // si on a trouv� tout le pr�fix
					{
